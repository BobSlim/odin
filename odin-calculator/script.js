  ...
  calcInput: function (inputRequest) {
    ...
    if (inputRequest.match(operationRegex)) {
      ...
      if (this.activeTerm() == this.operation && inputRequest == "-") {
        ...
        this.activeTerm() == this.firstTerm &&
        !this.firstTerm.innerText &&
        inputRequest == "-"
      ) {
        this.firstTerm.innerText += inputRequest;
      } else {
        this.operation.innerText = inputRequest;
      }
...
function setInputEventListeners(buttons) {
  for (i = 0; i < buttons.length; i++) {
    document.getElementById(buttons[i]).addEventListener("click", (event) => {
...
function handleKeyDown(keyRequest) {
  ...
  if (
    CALC.keys.number.includes(keyRequest) ||
    CALC.keys.operate.includes(keyRequest)
  ) {
