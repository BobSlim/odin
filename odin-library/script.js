...
class Book {
  ...
  generateDOM() {
    ...
    newBookDOM.innerHTML = `<div class="book-title">${this.title}</div>
            <div class="book-author">by ${this.author}</div>
            <div class="book-pages">Pages: ${this.pages}</div>
            <div class="book-buttons">
                <button class="book-readButton">Read:${
                  this.read ? "☒" : "☐"
                }</button>
    ...
    newBookDOM
      .querySelector("button.book-readButton")
      .addEventListener("click", this.toggleRead.bind(this));
    newBookDOM
      .querySelector("button.book-removeButton")
  ...
  updateDOMfields() {
    ...
    this.htmlObject.querySelector(
      ".book-author",
    ).innerText = `by ${this.author}`;
    this.htmlObject.querySelector(
      ".book-pages",
    ).innerText = `Pages: ${this.pages}`;
    this.htmlObject.querySelector(".book-readButton").innerText = `Read:${
      this.read ? "☒" : "☐"
    }`;
  }
}
